            ...
            if(!this.y_scaled) {
                ...
                for(let i = 0; i < this.lines.length; i++) {
                    for(let j = this.active_from; j < this.active_to; j++) {
                        if((min > this.lines[i].data[j] || min === null) && this.lines[i].is_active) {
        ...
        if(this.type == 'line') {
            ...
            if(this.y_scaled) {
                for(let i = 0; i < this.lines.length; i++) {
                    ...
                    ///this.addYAxis(max, true, !i ? 5 : this.width - 50, this.lines[i].colour, min);
                }
            }
            else {
                let min = null;
                ...
                this.addYAxis(this.findMaxValue(), true, 5, this.params['text_color'], min);
            }
            this.addXAxis();
        }
        else if(this.type == 'bar') {
...
//                scale = this.anime_params['bar_alpha']
    ...
    addYAxis(max, add_lines = true, x_margin = 5, text_colour = this.params['text_color'], min_value = 0) {
        const len = Math.trunc(max - min_value).toString().length;
        let multiplier = 5 * Math.pow(10, len - 2);
        const max_next = Math.ceil((max - min_value)/ multiplier) * multiplier;
        ...
        for(let i = 0; i <= total_steps; i++) {
            ...
            this.drawer.text(this.chart_ctx, formatNumber(multiplier * i + min_value), x_margin, y - y_margin, text_colour, this.anime_params['axis_alpha']);
        }
    }
