//import CanvNativeDrawer from './canvas_drawer';
...
class TgChart {
    ...
    initMapHandlers() {
        ...
        let map_min_width = Math.round(this.width / 16);
        map_caret.style.minWidth = map_min_width + 'px';
        ...
        const start_map_moving = e => {
            ...
            map_width = map_caret.offsetWidth;
        ...
        const moving_caret = e => {
            ...
            if(is_map_right_moving || is_map_left_moving) {
                let from = Math.round(map_caret.offsetLeft / this.map_step);
                let to = Math.round((map_caret.offsetLeft + map_caret.offsetWidth) / this.map_step);
        ...
        map_caret.addEventListener('touchstart', start_map_moving);

        document.addEventListener('mousemove', moving_caret);
        document.addEventListener('touchmove', moving_caret);
    ...
    getMapTemplate() {
        ...
        <div class="map">
            ...
            <div class="map-caret" id="${this.map_ids['caret']}">
    ...
    initPointerHandler() {
        ...
        const move_pointer = e => {
            ...
            //@todo не позволять вылазить за пределы графика (тестить на моб в право и влево)
            if(!e.touches) {
                //@todo показывать только внутри графика (сейчас попадает на миникарту)
                ...
                //console.log(xpointer_label.offsetTop, document.documentElement.scrollTop, this.chart.offsetTop + this.height, pointer_position_y);
            }
            ...
            if(!this.x.data[cur_point_index]) {
                hide_pointer();
                return;
            }

            let pointer_position_x = prev_point + this.chart.offsetLeft;
        ...
        document.addEventListener('touchmove', move_pointer);
    }
    ...
    calculatePointsActiveLinesPoints(max, axis_height, axis_offset, use_full_data, interpolate) {
        ...
        for (let i = 0; i < this.lines.length; i++) {
            ...
            let line_points = this.calculateLinePoints(this.lines[i], max, axis_height, axis_offset, use_full_data, interpolate);
     ...
     * @returns {Array}
     */
    calculateLinePoints(line, max, axis_height, axis_offset, use_full_data = false, interpolate = true) {
    ...
    findMaxValue(use_full_data = false) {
        ...
        if(active_lines.length == 1) {
            let max = 0;
            for(let i = 0; i < active_lines[0].data.length; i++) {
                if(max < active_lines[0].data[i]) {
                    max = active_lines[0].data[i];
                }
            }
            return max;
        }
        return active_lines.reduce((a, b) => {
            let b_data = use_full_data ? b.data : b.data.slice(this.active_from, this.active_to);
            let max_b = 0;
            ...
            let max_a = 0;
            if(!a || typeof a.data === 'undefined') {
                max_a = a;
            }
            else {
                let a_data = use_full_data ? a.data : a.data.slice(this.active_from, this.active_to);
